...
import { loginFailure, loginLoading, loginSuccess } from '../reducer/authReducer'
    ...
    const handleFormSubmit = async () => {
        ...
        try {
          ...
          if (response.data.success) {
            ...
            dispatch(loginSuccess(response.data.existingUser));
...
if(err.response.data.message.includes('password')){
  ...
  return (
    <div className='bg-gradient-to-b h-screen from-fuchsia-500 to-cyan-500'>
    <Navbar/>
    <div
    class="relative flex w-3/4 mx-auto flex-col rounded-xl bg-gradient-to-br  from-violet-200 to-pink-200 bg-clip-border text-gray-700 mt-11 shadow-md"
  >
    <div
      class="relative mx-4 -mt-6 mb-4 grid h-28 place-items-center overflow-hidden rounded-xl bg-gradient-to-tr  from-fuchsia-500 to-cyan-500 bg-clip-border text-white shadow-lg shadow-cyan-500/40"
    >
      <h3
    ...
    <div class="flex flex-col gap-4 p-6">
      ...
      {error && <span className='flex justify-center text-red-500 font-semibold md:-mt-7 text-sm md:text-base -mt-14 italic'>{error}</span>}
      </div>
    ...
    <div class="p-6 pt-0">
      <button
        ...
        type="button"
        class="block w-full select-none rounded-lg bg-gradient-to-tr from-fuchsia-500 to-cyan-500   py-3 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-white shadow-md shadow-cyan-500/20 transition-all hover:shadow-lg hover:shadow-cyan-500/40 active:opacity-[0.85] disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
